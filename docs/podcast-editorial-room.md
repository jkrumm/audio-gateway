...
| 4 | **Tool calling on the OpenAI dialect, research model = `deepseek-v4.1-flash`** (`reasoning_effort: high`), editorial + outline = `deepseek-v4.1-flash`, voice owner = `claude-opus-4-6`, reviewers `gemini-3.8-flash,deepseek-v4.1-flash`, metadata `deepseek-v4.1-flash`. All env-overridable (model AND effort, see `docs/podcast.md`). | 2026-09-13 model rollout consolidated the OpenAI-leg roles onto deepseek-v4.1-flash (`reasoning_effort` high, function tools alongside it — live-probed to accept both together, unlike gpt-5.6-luna's and gpt-6-luna's tools+reasoning_effort rejection on `/chat/completions`). Claude (voice owner) and Gemini (reviewer roster) never get `reasoning_effort` — `resolveReasoningEffort` (model-resolution.ts) omits it for both. Gemini on the compat route needs its `thought_signature` echoed back verbatim — the loop does that generically by echoing the assistant message untouched. |
